...
describe('', () => {
  ...
  it('determine example 1 15m distance point', () => {
    let shot = new Shot()
    let point = shot.determineZDistancePoint(pointExamples.example1.originPoint, pointExamples.example1.targetImpactPoint, 15);
  ...
  it('determine example 1 20m distance point', () => {
    let shot = new Shot()
    let point = shot.determineZDistancePoint(pointExamples.example1.originPoint, pointExamples.example1.targetImpactPoint, 20);
  ...
  it('determine example 2 20m distance point', () => {
    let shot = new Shot()
    let point = shot.determineZDistancePoint(pointExamples.example2.originPoint, pointExamples.example2.targetImpactPoint, 20);
  ...
  it('determine example 2 30m distance point', () => {
    let shot = new Shot()
    let point = shot.determineZDistancePoint(pointExamples.example2.originPoint, pointExamples.example2.targetImpactPoint, 30);
...
describe('', () => {
  ...
  it('create shot record', async () => {
    let database = new LeadDatabase(Settings.DatabaseConfig)
    await expect(database.saveLeadDeposit(examples.modelOne)).resolves.toEqual(true)
  })
})